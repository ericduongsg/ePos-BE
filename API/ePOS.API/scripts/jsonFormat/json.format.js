$(document).ready(function () {
    ...
    //Format request Json
    $(".requestJson").each(function () {
        if ($(this).html() != null) {
            var requestJsonContent = $(this).html();
            if (requestJsonContent.length > 0) {
                $(this).html("<h4>" + $(this).attr("title") + "</h4><textarea class='hide RawJson_0'>" + requestJsonContent + "</textarea><div class='Canvas_0' style='overflow:auto;'></div>");
                window.formatter = new QuickJSONFormatter({
                    RawJsonId: $(this).find(".RawJson_0"),
                    CanvasId: $(this).find(".Canvas_0"),
                    TabSize: 1,
                });
                formatter.Process();
            }
