    ...
    public class DevController : ApiController
    {
        ...
        public ApiResult GenerateResultJson(GenerateContentModelRequest model)
        {
            ...
            try
            {
                ...
                else if (apiKeyInfo.Type == "5")//Xiaomi MiMo
                {
                    return requestXiaomiMiMoModel(requestPrompt, apiKey, apiKeyInfo.Api_Key);
                }
                else
                {
        ...
        private ApiResult requestXiaomiMiMoModel(string requestPrompt, string apiKey, string encryptedApiKey)
        {
            var requestInfo = new
            {
                model = System.Configuration.ConfigurationManager.AppSettings["XIAOMI_MIMO_MODEL"].ToString(),
                temperature = 0,
                max_tokens = 4096,
                thinking =
                new
            ...
            var client = new RestClient(System.Configuration.ConfigurationManager.AppSettings["XIAOMI_MIMO_API_URL"].ToString());
            var request = new RestRequest(Method.POST);
            ...
            if (restResponse.StatusCode == System.Net.HttpStatusCode.OK)
            {
                ...
                _logger.Info("XiaomiMiMo response: " + textResponse);
            ...
            {
                var objSerialize = JsonConvert.SerializeObject(restResponse.Content, Formatting.Indented);
                _logger.Error("requestXiaomiMiMo Error encryptedApiKey: " + encryptedApiKey + ", responseMessage: " + Convert.ToString(objSerialize));
